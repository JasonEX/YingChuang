...
export function makeNovelsChapter(chapter = 21, page = 1, total = 3): string {
  ...
  const next = page < total ? novelsUrl(chapter, page + 1) : novelsUrl(chapter + 1);
  ...
  return `<!doctype html><html><head><title>第${chapter}章 山間行旅|繁體小說</title></head><body>
    <div class="text_title"><h1>第${chapter}章 山間行旅（${page} / ${total}）</h1>
    ...
    <div class="read_nav"><a id="prev_url" href="${prev}">上一${page === 1 ? '章' : '頁'}</a>
      ...
      <a id="next_url" href="https://ad.invalid/" data-real-href="${next}">下一${page === total ? '章' : '頁'}</a></div>
    </body></html>`;
}
