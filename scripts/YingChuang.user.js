...
`;
	...
	function prepareChapterLoad(ctx, direction, source) {
		...
		if (!rawTargetUrl || !refChapter) {
			if (source === "manual") ctx.showToast(refChapter?.sectionProgress ? SECTION_MERGING_TOAST : endMessage, "info");
			return null;
		}
					...
					createBaseVNode("main", {
						...
						(openBlock(true), createElementBlock(Fragment, null, renderList(displayChapters.value, (entry) => {
							return openBlock(), createElementBlock("article", {
								...
								entry.sectionProgress ? (openBlock(), createElementBlock("div", _hoisted_6, [createVNode(unref(MnrSpinner_default), { size: "small" }), createBaseVNode("span", null, toDisplayString(sectionProgressLabel(entry.sectionProgress)), 1)])) : createCommentVNode("", true)
							], 8, _hoisted_3);
						}), 128)),
						createBaseVNode("div", {
						...
						unref(readerStore).isLoadingNext ? (openBlock(), createElementBlock("div", _hoisted_7, [createVNode(unref(MnrSpinner_default), { size: "small" }), _cache[4] || (_cache[4] = createBaseVNode("span", null, "加载下一章...", -1))])) : createCommentVNode("", true),
						unref(readerStore).chapters.length > 0 && !unref(readerStore).hasNext && !unref(readerStore).isLoadingNext && !unref(readerStore).isTailSectionMerging ? (openBlock(), createElementBlock("div", _hoisted_8, [_cache[5] || (_cache[5] = createBaseVNode("p", { class: "mnr-chapter-end-text" }, "— 已是最后一章 —", -1)), createBaseVNode("div", _hoisted_9, [unref(readerStore).chapter?.indexUrl ? (openBlock(), createElementBlock("a", {
							key: 0,
