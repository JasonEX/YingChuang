...
`;
	...
	function getSimplifiedConverter() {
		if (!simplifiedConverter) {
			const sourceCharacters = new Set(TSCharacters_default.split("|").flatMap((entry) => {
				const [source, target] = entry.split(" ");
				return source === target ? [] : [source];
			}));
			const phrases = TSPhrases_default.split("|").filter((entry) => {
				const [source, target] = entry.split(" ");
				const from = [...source];
				const to = [...target];
				return from.length === to.length && from.every((char, index) => char === to[index] || sourceCharacters.has(char));
			});
			const preservedTargets = phrases.map((entry) => entry.split(" ")[1]).filter((target) => [...target].some((char) => sourceCharacters.has(char))).map((target) => `${target} ${target}`);
			simplifiedConverter = ConverterFactory([CJK_Compatibility_Ideographs_default], [[...phrases, ...preservedTargets].join("|"), TSCharacters_default]);
		}
	...
	async function convertText(text, mode, options = {}) {
		...
		try {
			return mode === "sc" ? getSimplifiedConverter()(text) : r(text);
		} catch (error) {
	...
	async function convertHTML(html, mode, options = {}) {
		...
		try {
			const converter = mode === "sc" ? getSimplifiedConverter() : r;
			const template = document.createElement("template");
	...
	async function appendChapterSection(ctx, entryId, delta) {
		...
		if (mode !== merge.convertedMode || mode === "tc" && delta.sourceScript !== merge.convertedScript) await ctx.applyConversionToChapterEntry(entryId, mode);
		else {
	...
	async function reconcileMergedConversion(ctx, entryId, entry, merged, convertedScript, convertedMode) {
		...
		if (convertedMode !== mode || mode === "tc" && convertedScript !== merged.sourceScript) await ctx.applyConversionToChapterEntry(entryId, mode);
	}
